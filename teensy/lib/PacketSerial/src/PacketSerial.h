...
public:
    ...
    /// \brief Deprecated. Use setStream() to configure a non-default port.
    /// \param stream A pointer to an teensy `Stream`.
    /// \deprecated Use setStream() to configure a non-default port.
    ...
    /// \brief Attach PacketSerial to an existing teensy `Stream`.
    ///
    ...
    void update()
    {
        ...
        while (_stream->available() > 0)
        {
            ...
            if (data == PacketMarker)
            {
                ...
                handlePacket(_decodeBuffer, numDecoded);
...
private:
    ...
    // ESB Added to replace _onPacketFunction and _onPacketFunctionWithSender
    virtual void handlePacket(const uint8_t* buffer, size_t size) = 0;
